...
// Cek apakah user adalah pemilik atau moderator
$auth_check = $conn->prepare("SELECT role FROM circle_members WHERE circle_id = ? AND user_id = ?");
$auth_check->bind_param("ii", $circle_id, $user_id);
$auth_check->execute();
$auth_check->bind_result($role);

if (!$auth_check->fetch() || !in_array($role, ['moderator', 'member']) && $role !== null) {
...
<div class="container mt-5">
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0">Permintaan Gabung Circle</h4>
        </div>
        <div class="card-body">
                    ...
                    <?php while ($row = $requests->fetch_assoc()): ?>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center">
                                ...
                                <small class="text-muted ms-2"><?= $row['created_at'] ?></small>
                            </div>
                            <div class="d-flex gap-2">
                                <form method="POST" action="../backend/circle/handle_request.php">
                                    <input type="hidden" name="circle_id" value="<?= $circle_id ?>">
                                    <input type="hidden" name="request_id" value="<?= $row['id'] ?>">
                                    <input type="hidden" name="user_id" value="<?= $row['user_id'] ?>">
                                    <button name="action" value="approve" class="btn btn-success btn-sm">✔️</button>
        ...
        <div class="card-footer text-end">
            <a href="discussion_page.php?circle_id=<?= $circle_id ?>" class="btn btn-secondary">Kembali ke Diskusi</a>
        </div>
    </div>
</div>
