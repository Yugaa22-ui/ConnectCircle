...
// Validasi: hanya creator atau moderator
$auth = $conn->prepare("
    SELECT cm.role, c.creator_id 
    FROM circle_members cm 
    JOIN circles c ON c.id = cm.circle_id 
    WHERE cm.user_id = ? AND cm.circle_id = ?
");
...
if ($action === 'approve') {
    ...
    $delete = $conn->prepare("DELETE FROM circle_requests WHERE id = ?");
